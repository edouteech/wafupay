...
import { Bell, Faq, Home, Logout, Params, TransCash, User } from "../../(dashboard)/Components/icons";
        ...
        <>
            <div className="relative text-[19px] bg-bodyBg text-white overflow-hidden h-[100vh] w-[100vw]">
                <div className="absolute left-0 top-0 bottom-0 w-64 z-20">
                    <ul className="py-2 text-textGray bg-white flex-col flex gap-6 h-full border-r">
                    <div className="p-4 bg-primary">
                        <Image alt="logo" className="w-[143px] m-auto bg-white rounded p-2" src={logo}></Image>
                    </div>
                        <li className=" flex gap-2 h-16 font-bold text-gray-700"><span className="rounded-r-2xl bg-primary h-full w-2"> </span><Link className={`${href[3] == "dashboard" || href[3] == "verify" ? 'text-primary' : ''} flex items-center gap-3`} href={'/dashboard'}><Home f={`${href[3] == "dashboard" || href[3] == "verify" ? '#1877F2' : '#606060'}`}></Home>Tableau de bord</Link></li>
