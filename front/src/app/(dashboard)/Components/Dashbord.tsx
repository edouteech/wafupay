        ...
        <>
            <div className="relative text-[19px] bg-bodyBg text-white overflow-hidden h-[100vh] w-[100vw]">
                ...
                <div className="flex bg-white p-2 items-center absolute top-0 h-24 pl-56 w-full z-10">
                    <div className="flex items-center gap-4 bg-white w-full p-2 text-primary rounded-lg justify-end xs:hidden">
                        <Bell></Bell>
                        <Image alt="belle femme" src={userPic}></Image>
                    </div>
                    <div className="xs:flex items-center gap-4 bg-white w-full p-2 text-primary rounded-lg justify-end hidden">
                        <button onClick={() => { setIsOpen(!isOpen) }}><Menu className="w-12 h-12"></Menu></button>
                    </div>

                </div>
                <ul className={`${isOpen ? 'h-72 py-2' : 'h-[0px]'} absolute z-20 text-textGray bg-white px-8 flex-col flex gap-6 w-full border-r mt-24 text-base overflow-hidden duration-500`}>
                    <li className=""><Link className={`${href[1] == "dashboard" || href[1] == "verify" ? 'text-primary' : ''} flex items-center gap-3 `} href={'/dashboard'}><Home f={`${href[1] == "dashboard" || href[1] == "verify" ? '#1877F2' : '#606060'}`}></Home>Dashboard</Link></li>
                    <li className=""> <button onClick={() => { setShowTrans(!showTrans) }} className={`${href[1] == "transactions" ? 'text-primary' : ''} flex items-center gap-3 `}><TransCash f={`${href[1] == "transactions" ? '#1877F2' : '#606060'}`} ></TransCash>Transactions</button>
                        <ul className={`${showTrans ? 'h-24 mt-4' : 'h-0'} ml-8 text-sm flex flex-col gap-2 duration-500 overflow-hidden`}>
                            <li className={`${href[1] == "transactions" && href[2] == 'mobile' ? 'text-primary' : ''} list-disc hover:text-primary`}><Link href={'/transactions/mobile'}>Transfert par mobile</Link></li>
                            <li className={`${href[1] == "transactions" && href[2] == 'carte' ? 'text-primary' : ''} list-disc hover:text-primary`}><Link href={'/transactions/carte'}>Transfert par carte</Link></li>
                            <li className={`${href[1] == "transactions" && href[2] == 'historique' ? 'text-primary' : ''} list-disc hover:text-primary`}><Link href={'/transactions/historique'}>Historique</Link></li>
                    ...
                    <li className=""><Link className={`${href[1] == "profil" ? 'text-primary' : ''} flex items-center gap-3 `} href={'/profil'}><User f={`${href[1] == "profil" ? '#1877F2' : '#606060'}`}></User>Profil</Link></li>
                    <li className=""><Link className={`${href[1] == "params" ? 'text-primary' : ''} flex items-center gap-3 `} href={'/params'}><Params f={`${href[1] == "params" ? '#1877F2' : '#606060'}`}></Params>Paramètres</Link></li>
                    <li className=""><Link className={`${href[1] == "faq" ? 'text-primary' : ''} flex items-center gap-3 `} href={'/faq'}><Faq f={`${href[1] == "faq" ? '#1877F2' : '#606060'}`}></Faq>FAQ</Link></li>
                    <li className=""><button className={` flex items-center gap-3 `} onClick={() => { logout() }}><Logout></Logout>Déconnexion</button></li>
                </ul>
                <div className="pl-60 pt-24 xs:px-4 relative w-[100vW] h-[100vh] overflow-y-scroll text-textGray">
                    {children}
                </div>
            </div >
        </>
